...
export default function LandlordDashboard() {
      ...
      <main style={{ minHeight: '100vh', background: '#F8FAFC' }}>
        ...
        <div style={{ paddingTop: '5rem', display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '60vh' }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
            <div style={{ width: 48, height: 48, borderRadius: '50%', border: '4px solid #E2E8F0', borderTopColor: '#0F172A', animation: 'spin 0.8s linear infinite' }} />
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#F8FAFC' }}>
      ...
      {/* ── Header ─────────────────────────────────────────── */}
      <div style={{ background: '#020617', padding: '7rem 1.5rem 1.75rem' }}>
            ...
            {[
              ...
              <button key={key} id={`landlord-tab-${key}`} onClick={() => setActiveTab(key)}
                style={{
                  ...
                  color: activeTab === key ? '#10B981' : '#888',
          ...
          <div style={{ padding: '1.25rem' }}>
              ...
              <div id="my-listings">
                {properties.length === 0 ? (
                  <div style={{ textAlign: 'center', padding: '3rem', color: '#aaa' }}>
                    ...
                    <button onClick={() => setActiveTab('add')} style={{ background: '#10B981', color: '#fff', border: 'none', borderRadius: 8, padding: '8px 18px', fontWeight: 600, cursor: 'pointer', fontSize: '0.85rem' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                    {properties.map(p => (
                      <div key={p.id} style={{ display: 'flex', gap: 12, padding: 12, background: '#F8FAFC', borderRadius: 8, border: '1px solid #E2E8F0', alignItems: 'center' }}>
                        <div style={{ position: 'relative', width: 80, height: 60, borderRadius: 6, overflow: 'hidden', flexShrink: 0, background: '#E2E8F0' }}>
                          {p.images && p.images[0] ? (
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                            <p style={{ fontWeight: 600, fontSize: '0.875rem', color: '#0F172A', overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>
                              {lang === 'fr' ? p.title : (p.title_en || p.title)}
                            </p>
                            {p.verified && <span style={{ flexShrink: 0, background: '#e8f2ee', color: '#0F172A', fontSize: '0.7rem', fontWeight: 600, padding: '2px 6px', borderRadius: 4 }}>✓ {lang === 'fr' ? 'Vérifié' : 'Verified'}</span>}
                          </div>
                          <p style={{ color: '#aaa', fontSize: '0.75rem', marginBottom: 3 }}>{p.neighborhood} · {p.bedrooms} {lang === 'fr' ? 'ch.' : 'bd.'} · {p.area}m²</p>
                          <p style={{ fontWeight: 700, color: '#0F172A', fontSize: '0.85rem' }}>{formatPrice(p.price)}/{lang === 'fr' ? 'mois' : 'mo'}</p>
                        </div>
              ...
              <div id="booking-requests" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {bookings.length === 0 ? (
                  ...
                  bookings.map(booking => {
                    ...
                    return (
                      <div key={booking.id} style={{ padding: 14, background: '#F8FAFC', borderRadius: 8, border: `1px solid ${booking.status === 'pending' ? '#fde68a' : booking.status === 'confirmed' ? '#86efac' : '#E2E8F0'}` }}>
                        <div style={{ display: 'flex', gap: 10, marginBottom: 10 }}>
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2, flexWrap: 'wrap' }}>
                              <p style={{ fontWeight: 700, fontSize: '0.875rem', color: '#0F172A' }}>{tenant?.full_name ?? '—'}</p>
                              {tenant?.trust_score ? (
                                <span style={{ display: 'flex', alignItems: 'center', gap: 3, fontSize: '0.78rem', color: '#555' }}>
                                  <Star size={11} color="#10B981" fill="#10B981" /> {tenant.trust_score.toFixed(1)}
                                </span>
                              ) : null}
                              <span style={{
                            ...
                            {booking.message && (
                              <p style={{ color: '#666', fontSize: '0.8rem', fontStyle: 'italic', marginTop: 5, background: '#fff', borderRadius: 6, padding: '5px 8px', border: '1px solid #E2E8F0' }}>
                                &ldquo;{booking.message}&rdquo;
                              </p>
                            )}
                            {tenant?.phone && (
                              <a href={`tel:${tenant.phone}`} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, marginTop: 6, fontSize: '0.78rem', color: '#0F172A', fontWeight: 600, textDecoration: 'none' }}>
                          ...
                          <div style={{ display: 'flex', gap: 8 }}>
                            ...
                            {tenant?.email && (
                              <a href={`mailto:${tenant.email}`}
                                style={{ padding: '8px 12px', background: '#fff', border: '1px solid #E2E8F0', borderRadius: 6, fontSize: '0.8rem', fontWeight: 600, color: '#0F172A', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4, textDecoration: 'none' }}>
                                <Phone size={12} />
                              </a>
                            )}
                          </div>
            ...
            {activeTab === 'add' && (
              <div id="add-property-form">
                <h2 style={{ fontWeight: 700, fontSize: '1rem', color: '#0F172A', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: 6 }}>
                  <PlusCircle size={16} color="#10B981" /> {lang === 'fr' ? 'Publier un nouveau bien' : 'Publish a new property'}
                </h2>
                ...
                {addStatus === 'success' ? (
                  ...
                  <form onSubmit={handleAddProperty} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                      ...
                      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                        {PROPERTY_TYPES.map(pt => (
                          <button key={pt.value} type="button" onClick={() => setAddForm(f => ({ ...f, property_type: pt.value }))}
                            style={{ padding: '7px 14px', borderRadius: 8, border: `1.5px solid ${addForm.property_type === pt.value ? '#10B981' : '#E2E8F0'}`, background: addForm.property_type === pt.value ? '#fdf3ee' : '#fff', color: addForm.property_type === pt.value ? '#10B981' : '#555', fontWeight: 600, fontSize: '0.82rem', cursor: 'pointer' }}>
                      ...
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                        {AMENITIES_FR.map((a, idx) => (
                          <button key={a} type="button" onClick={() => toggleAmenity(idx)}
                            style={{ padding: '5px 12px', borderRadius: 99, border: `1.5px solid ${selectedAmenities.includes(idx) ? '#0F172A' : '#E2E8F0'}`, background: selectedAmenities.includes(idx) ? '#e8f2ee' : '#fff', color: selectedAmenities.includes(idx) ? '#0F172A' : '#666', fontSize: '0.78rem', fontWeight: 600, cursor: 'pointer', transition: 'all 0.15s' }}>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(100px, 1fr))', gap: 8 }}>
                        ...
                        {images.length < 8 && (
                          <button type="button" onClick={() => fileRef.current?.click()}
                            style={{ aspectRatio: '4/3', border: '2px dashed #d1cfc9', borderRadius: 8, background: '#F8FAFC', cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 6, color: '#aaa' }}>
                            <Upload size={20} />
                    ...
                    <button id="add-submit" type="submit"
                      ...
                      style={{ padding: '0.9rem', background: (addStatus === 'uploading' || addStatus === 'saving') ? '#888' : '#10B981', color: '#fff', border: 'none', borderRadius: 8, fontWeight: 700, fontSize: '0.95rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
                      {addStatus === 'uploading' ? (
